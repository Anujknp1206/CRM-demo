...
import { fetchPages } from "../features/functions";
import { BASE_URL } from "../utils/config";
import { useLocation } from "react-router-dom";
import { ContactInfo, ContactForm, Faq, NotFound } from "../components";
import { Banner, RouteMap } from "../sections";
...
function Page() {
  ...
  const { pagesData, error } = useSelector((state) => state.pagesData);

  const pageSlug = location.pathname.split("/").filter(Boolean).pop();

  const [pageId, setPageId] = useState(null);

  useEffect(() => {
    if (menuItems.length > 0) {
      const matched = menuItems.find((item) => item.slug === pageSlug);
      if (matched) {
        setPageId(matched.link_value);
        dispatch(fetchPages(matched.link_value));
      }
    }
  }, [dispatch, menuItems, pageSlug]);
  ...
  const hasBgColor =
    !!bannerContent.bg_color &&
    bannerContent.bg_color.toLowerCase() !== "#ffffff";
      ...
      <div className="w-full h-64 relative overflow-hidden">
        ...
        {hasImage && (
          <div
            ...
            style={{
              backgroundImage: `url(${BASE_URL}/${bannerContent.image})`,
            }}
          />
        )}
          ...
          {sections.map((section) => {
            switch (true) {
              ...
              case section.type === "faq":
                return (
                  <div className="col-span-2 px-10" key={`faq-${section.id}`}>
